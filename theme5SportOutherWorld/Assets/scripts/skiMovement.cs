...
public class skiMovement : MonoBehaviour
{
    ...
    public float boostDuration = 3f; // Time for boost to last (in seconds)
    public float boostMultiplier = 1.5f; // Boost strength multiplier
    ...
    void Start()
    {
        ...
        originalGravityStrength = gravityStrength;
    ...
    public void speedBoost(float trickPoints)
    {
        if (!isBoosting)
        {
            StartCoroutine(boostCoroutine(trickPoints));
        }
    }
    IEnumerator boostCoroutine(float trickPoints)
    {
        isBoosting = true;

        // Temporary Boost Values
        gravityStrength *= boostMultiplier;
        acceleration *= boostMultiplier;

        Debug.Log("Boost Applied: Gravity = " + gravityStrength + ", Acceleration = " + acceleration);

        // Wait for boost duration
        yield return new WaitForSeconds(boostDuration);

        // Reset to normal values after boost duration
        gravityStrength = originalGravityStrength;
        acceleration = originalAcceleration;

        Debug.Log("Boost Ended: Gravity = " + gravityStrength + ", Acceleration = " + acceleration);
