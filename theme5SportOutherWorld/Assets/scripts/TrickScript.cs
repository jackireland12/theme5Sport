...
public class TrickScript : MonoBehaviour
{
    ...
    public float comboValue = 600f;

    // Start is called before the first frame update
    void Start()
    {
        sk = GetComponentInParent<skiMovement>();
        //tu = GetComponentInParent<trickUI>();
    }
    ...
    public void crashCheack() 
    {
        ...
        if (Mathf.Abs(tiltX) > maxTilt || Mathf.Abs(tiltZ) > maxTilt)
        {
           ...
           crash();
           tu.ShowLanding(false);
        }
        else
        {
            Debug.Log("Perfect landing!");
            if (trickRotation >= comboValue)
            {
                trickCombo++;
            ...
            trickRotation = 0;
            trickint = Mathf.RoundToInt(trickScore);
            sk.speedBoost(trickRotation);
            tu.UpdateScore(trickint);
            tu.UpdateCombo(trickCombo);
            tu.ShowLanding(true);
        }
